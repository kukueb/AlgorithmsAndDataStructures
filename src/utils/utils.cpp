...
void remove_prime_between_even(list* beg) {
  list *cur = beg->next;
  while (cur->next != NULL) {
    if (is_prime(cur->info) && cur->prev->info%2 == 0 && cur->next->info%2 == 0) {
...
bool is_sorted(list* beg) {
  ...
  while (t->prev != NULL) {
    if (t->prev->info > t->info) return 0; 
    t = t->prev;
  }
  return 1;
}
...
list* make_without_nonideal(list* beg) {
  ...
  while (c != NULL) {
    if (is_ideal(c)) push_next(&new_beg, c);
    c = c->next;
  }
  while (new_beg->prev != NULL) new_beg = new_beg->prev;
  return new_beg;
}
